...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  if (cmd->word_count == 1) {
    ...
    //If correct amount of args given, change current location
  ...
  if (chdir(target_dir) != 0){
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: ERROR\n");
    return -1;
  }
  if (vars_set("PWD", target_dir) < 0){
    dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: failed to update PWD\n");
    return -1;
  }
  //chdir(target_dir);
  return 0;
