...
signal_init(void)
{
  if (sigaction(SIGTSTP, &interrupt_action, &old_sigtstp) == -1) { //ctrl Z
    return -1;
  }; 
  if (sigaction(SIGINT, &interrupt_action, &old_sigint) == -1){ //ctrl C
    return -1;
  }; 
  if (sigaction(SIGTTOU, &interrupt_action, &old_sigttou) == -1){ //When a background process writes to the terminal
    return -1;
  }; 
